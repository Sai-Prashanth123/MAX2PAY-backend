...
-- STEP 1: Fix inventory data inconsistencies
UPDATE inventory
...
-- Now safe to drop invoiced_in
ALTER TABLE orders DROP COLUMN IF EXISTS invoiced_in CASCADE;

-- STEP 4: Alter invoice columns (only if they exist)
DO $$ 
BEGIN
...
-- STEP 5: Atomic inventory function
...
-- STEP 11: Inventory audit trigger
...
-- STEP 12: Safe cancellation function
...
BEGIN
  ...
  LOOP
    ...
    IF NOT FOUND THEN
      RAISE EXCEPTION 'Cannot cancel order: insufficient reserved stock';
    END IF;
  END LOOP;
...
SELECT 
  ...
  (SELECT COUNT(*) FROM inventory WHERE available_stock < 0 OR reserved_stock < 0) as negative_inventory,
