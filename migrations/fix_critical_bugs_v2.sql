-- ============================================
-- CRITICAL BUG FIXES - Database Layer (v2)
-- Fixed: Handles existing inconsistent data
-- ============================================
...
-- STEP 3: Now safe to add constraints
...
FROM inventory 
WHERE available_stock < 0 OR reserved_stock < 0 OR dispatched_stock < 0;

-- Check for duplicate order numbers (should return 0 rows)
SELECT COUNT(*) as duplicate_orders_count
FROM (
...
SELECT COUNT(*) as inconsistent_stock_count
FROM inventory 
WHERE total_stock != (available_stock + reserved_stock + dispatched_stock);
